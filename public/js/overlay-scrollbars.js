...
(() => {
  ...
  function initializeOverlayScrollbars() {
    ...
    function scan() {
      ...
      scanTimer = 0;
      const candidates = /* @__PURE__ */ new Set([document.scrollingElement]);
      ...
      for (const entry of entries.values()) {
        ...
        for (const axis of ["x", "y"]) {
          ...
          track.classList.toggle(
            "is-visible",
            Boolean(
              visible && (entry.activeUntil > now || target.contains(hovered) || !isRoot(target) && target.contains(document.activeElement) || drag?.entry === entry)
            )
          );
        }
      }
