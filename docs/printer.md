...
- `src/app/receipt.js`：可選 24／28／32／40 點系統中文字型（預設 32 點），逐行換行，每段最多 238 點高；預覽分頁且同時最多四個 Canvas，送印依相同排版逐段產生黑白點陣，收到 ACK 後才產生下一段。不合併整筆 bitmap 到記憶體。預覽分頁不影響連續出紙。
- 單據頂端移除原有 4 點排版留白；尾端在原本進紙量上增加半行（依字體行高向上取整，預設 23 點）。半切參數由 16 增加為 `16 + 半行`；不切紙時先以 `ESC J` 多進半行，再保留原本 4 行進紙。尾端進紙不顯示於點陣預覽；切刀與列印頭之間的實機留白仍需出紙確認。
